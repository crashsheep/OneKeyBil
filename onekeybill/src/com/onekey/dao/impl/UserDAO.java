...
public class UserDAO extends BaseHibernateDAO {
	...
	public User findById(java.lang.Integer id) {
		...
		try {
			User instance = (User) getSession().get("com.onekey.entity.User",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<User> vali(String username,String password) {
		log.debug("开始登陆验证");
		try {
			String hql="from User where name = '"+username+"' AND password = '"+password+"'";
			List<User> results=getSession().createQuery(hql).list();
			return results;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
	...
	public List<User> findByExample(User instance) {
		...
		try {
			List<User> results = getSession()
					.createCriteria("com.onekey.entity.User")
